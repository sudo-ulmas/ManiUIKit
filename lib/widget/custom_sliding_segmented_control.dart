...
// ignore_for_file: comment_references, lines_longer_than_80_chars, prefer_asserts_with_message
...
const double _kMinSegmentedControlHeight = 38;
...
const EdgeInsets _kSeparatorInset = EdgeInsets.symmetric(
  vertical: AppDimensions.gap,
);
...
class CustomCupertinoSlidingSegmentedControl<T> extends StatefulWidget {
  ...
  })  : assert(children.length >= 2, 'children must be more than 1'),
        assert(
...
class _RenderSegmentedControl<T> extends RenderBox
  ...
  double computeMinIntrinsicWidth(double height) {
    ...
    var maxMinChildWidth = 0;
    while (child != null) {
      final childWidth = child.getMinIntrinsicWidth(height);
      maxMinChildWidth = math.max(maxMinChildWidth, childWidth.ceil());
  ...
  double computeMaxIntrinsicWidth(double height) {
    final childCount = this.childCount ~/ 2 + 1;
    var child = firstChild;
    var maxMaxChildWidth = 0;
    while (child != null) {
      final childWidth = child.getMaxIntrinsicWidth(height);
      maxMaxChildWidth = math.max(maxMaxChildWidth, childWidth.ceil());
      child = nonSeparatorChildAfter(child);
    }
  ...
  void performLayout() {
    ...
    while (child != null) {
      ...
      final childOffset = Offset(start.toDouble(), 0);
      childParentData.offset = childOffset;
      start += child.size.width.ceil();
      assert(
        index.isEven ||
